...
class App(tk.Tk):
    def __init__(self):
        ...
        self.current_text = self.search.get_default_data()
        self.old_rate = 1
        self.main_menu()
    ...
    def main_menu(self):
        ...
        tk.Button(self, text="EV insight",
                  ...
                  command=self.switch_insight_menu,
                  font=("Arial", 14), background="#4CAF50",
                  foreground="#ffffff"). \
        ...
        tk.Button(self, text="Compare EV cars",
                  ...
                  font=("Arial", 14), background="#FFC107",
                  foreground="#ffffff"). \
        ...
        tk.Button(self, text="Show Statistic of data",
                  relief="flat", width=20, height=2,
                  command=self.swift_stat_menu,
                  font=("Arial", 14), background="#2196F3",
                  foreground="#ffffff"). \
        ...
        tk.Button(self, text="Quit",
                  ...
                  font=("Arial", 14), background="#F44336",
                  foreground="#ffffff"). \
    ...
    def insight_menu(self):
        ...
        tk.Button(self, text='Cheapest', width=20, height=2,
                  command=self.button_cheapest,
                  relief="flat", font=("fantasy", 11), background="#42CCC9",
                  foreground="#FFFFFF"). \
            grid(row=0, column=0, padx=10, pady=10)

        tk.Button(self, text='Fast Charge', width=20, height=2,
                  command=self.button_fast_charge,
                  relief="flat", font=("fantasy", 11), background="#4C99CF",
                  foreground="#FFFFFF") \
            .grid(row=0, column=1, padx=10, pady=10)

        tk.Button(self, text='Most Efficient', width=20, height=2,
                  command=self.button_most_efficient,
                  relief="flat", font=("fantasy", 11), background="#4873A6",
                  foreground="#FFFFFF") \
            .grid(row=0, column=2, padx=10, pady=10)

        tk.Button(self, text='Longest Range', width=20, height=2,
                  command=self.button_longest_range,
                  relief="flat", font=("fantasy", 11), background="#5A5388",
                  foreground="#FFFFFF") \
            .grid(row=0, column=3, padx=10, pady=10)

        tk.Label(self, text='Change Price', width=10, height=2,
                 background="#E2D5F5",
        ...
        tk.Label(self, text='Search Car name', width=15, height=1,
                 background="#E2D5F5",
                 relief="flat", font=("fantasy", 15), foreground="#524364") \
        ...
        tk.Button(self, text='Search', width=20, height=1,
                  font=("fantasy", 10),
                  relief="flat", background="#56426A", foreground="#FFFFFF",
                  command=lambda: self.button_search()) \
            .grid(row=1, column=5, padx=(30, 0), pady=10)

        # SHOW ALL DATA
        self.treeview = ttk.Treeview()
        self.treeview.grid(row=2, columnspan=6, padx=(10, 0), sticky="NSEW")
        self.update_databox(self.search.get_default_data())
        ...
        tk.Button(self, text="Go back to Main Menu",
                  relief="flat", width=20, height=2,
                  font=("fantasy", 11), background="#91A486",
                  foreground="#FFFFFF", command=self.switch_main_menu) \
            .grid(row=3, columnspan=5, padx=10, pady=10)

        tk.Button(self, text='Reset', width=20, height=2,
                  relief="flat", font=("fantasy", 11), background="#C49346",
                  foreground="#FFFFFF", command=self.reset) \
            .grid(row=3, column=5, padx=10, pady=10)

    def on_double_click(self, event):
        item = self.treeview.selection()[0]
        car_name = self.treeview.item(item, "values")[1]
    ...
    def button_search(self):
        if self.Entry_search.get().lower():
            update_txt = self.search.search_data(
                self.Entry_search.get().lower())
    ...
    def compare_menu(self):
        ...
        self.car1 = ttk.Combobox(self, textvariable=default1,
                                 values=self.search.get_car_name_list())
        self.car1.grid(row=1, column=0, columnspan=2, padx=10, pady=10,
                       sticky="NSEW")
        self.car1.bind("<<ComboboxSelected>>", self.car_clicked)
        self.car2 = ttk.Combobox(self, textvariable=default2,
                                 values=self.search.get_car_name_list())
        self.car2.grid(row=1, column=2, columnspan=2, padx=10, pady=10,
                       sticky="NSEW")
    ...
    def car_clicked(self, event):
        if self.car1.get() and self.car2.get():
            self.plot_car_attributes(self.car1.get(), self.car2.get())

    def plot_car_attributes(self, car1, car2):
        attributes = [
            self.graph_plot.plot_price, self.graph_plot.plot_battery,
            self.graph_plot.plot_charge, self.graph_plot.plot_range,
            self.graph_plot.plot_top_speed, self.graph_plot.plot_acceleration,
            self.graph_plot.plot_efficiency
        ]
        for idx, plot_func in enumerate(attributes):
            plot_func(car1, car2).get_tk_widget().grid(row=2 + idx // 4,
                                                       column=idx % 4, padx=10,
                                                       pady=10, sticky="NSEW")
    ...
    def stat_menu(self):
        ...
        self.graph_plot.plot_histogram("Price.DE.").get_tk_widget(). \
            grid(row=1, column=0, padx=10, pady=10, sticky="NSEW")

        self.graph_plot.plot_correlation().get_tk_widget(). \
            grid(row=1, column=1, padx=10, pady=10, sticky="NSEW")
